...
mod tests {
    ...
    fn parse_challenge() {
        ...
        let challenge = ChallengeMsg::parse(&raw_challenge).expect("can't read NTLM challenge");

        assert_eq!(
            challenge.flags,
            Flags::UNICODE | Flags::NTLM | Flags::TARGET_TYPE_DOMAIN | Flags::TARGET_INFO
        );

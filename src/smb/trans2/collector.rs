...
impl CollectTrans2 {
    ...
    pub(crate) fn add(&mut self, msg: Transact2) -> Result<bool, Error> {
        self.parameter
            .add(msg.parameter, msg.parameter_offset, msg.parameter_total)?;
...
impl Collector {
    ...
    fn add(&mut self, data: Bytes, offset: usize, total: usize) -> Result<(), Error> {
        if let Some(saved_total) = self.total {
            // safety check: if server changes it's mind about total buffer
            // size, we bail!
            if saved_total != total {
                return Err(Error::Reassemble(format!(
                    "server changed it's mind about total packet size: {} vs {}",
                    saved_total, total
                )));
            }
        } else {
        ...
        if self.fragments.insert(offset, data).is_some() {
            return Err(Error::Reassemble(format!(
                "got fragment at offset {} twice",
                offset
            )));
        }
    ...
    fn len(&self) -> usize {
        self.fragments.values().map(|b| b.len()).sum()
    }
    ...
    fn get(mut self) -> Result<Bytes, Error> {
        ...
        if self.len() != total {
            return Err(Error::Reassemble(format!(
                "length does not match, want {}, but got {}",
                total,
                self.len()
            )));
        }

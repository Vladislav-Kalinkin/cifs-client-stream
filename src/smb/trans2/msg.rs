...
impl<T: SubCmd> Msg for Transact2<T> {
    ...
    fn body(
        ...
        let sub_parameter_len: u16 = sub_parameter.len().try_into().map_err(|_| {
            Error::CreatePacket("parameter transaction2 sub-command is too large".to_owned())
        })?;

        let sub_data = self.subcmd.data()?;
        let sub_data_len: u16 = sub_data.len().try_into().map_err(|_| {
            Error::CreatePacket("data of transaction2 sub-command is too large".to_owned())
        })?;
        ...
        parameter.put_u16_le(sub_parameter_len);
        parameter.put_u16_le(
            sub_parameter_offset
                .try_into()
                .expect("parameter offset too big"),
        );
